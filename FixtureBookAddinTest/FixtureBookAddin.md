...
1.  事前にデータベース設定ファイル(db.config)を削除しておくこと。
    *   削除した
...
### テーブル追加（データ取得）
既存シートにデータベースから取得したデータでテーブル追加できる

1.  事前にデータベース設定を行っておくこと
    *   設定画面で`System.Data.SqlClient`（SQLServer 用のプロバイダ）を選択しておく
...
1.  以降のテストのためにSQLServerに1000件データのロードを行う
    *   FixtureBookAddinTest の SQLServer__1000件データ を実行し、
        SQLServer上の`TypesTable`に1000件のデータをロードする
1.  クエリーウインドウでテーブルを選択せずに、SQL入力用テキストフィールドを未入力のまま
    「クエリー実行」ボタンをクリックするとウインドウがクローズする
...
1.  クエリーウインドウで取得件数を空白にして、`TypesTable`を選択し、「クエリー実行」をクリックすると、
    10件のデータが追加される
1.  以降のテストのためにSQLServerに0件データのロードを行う
    *   FixtureBookAddinTest の SQLServer__0件データ を実行し、
        SQLServer上の`TypesTable`に0件のデータをロードする
1.  クエリーウインドウで `TypesTable`を選択し、「クエリー実行」をクリックすると、
    2件のデータが追加され、DBから取得したデータとしては何も表示されない。
1.  以降のテストのためにSQLServerに1件データのロードを行う
    *   FixtureBookAddinTest の SQLServer__1件データ を実行し、
        SQLServer上の`TypesTable`に1件のデータをロードする
1.  クエリーウインドウで `TypesTable`を選択し、「クエリー実行」をクリックすると、
    2件のデータが追加され、1件目にDBから取得したデータが表示される
1.  以降のテストのためにSQLServerに2件データのロードを行う
    *   FixtureBookAddinTest の SQLServer__2件データ を実行し、
        SQLServer上の`TypesTable`に2件のデータをロードする
1.  クエリーウインドウで `TypesTable`を選択し、「クエリー実行」をクリックすると、
    2件のデータが追加され、1件目, 2件目にDBから取得したデータが表示される
1.  以降のテストのためにSQLServerに3件データのロードを行う
    *   FixtureBookAddinTest の SQLServer__3件データ を実行し、
        SQLServer上の`TypesTable`に3件のデータをロードする
1.  クエリーウインドウで `TypesTable`を選択し、「クエリー実行」をクリックすると、
    3件のデータが追加され、1件目, 2件目, 3件目にDBから取得したデータが表示される
1.  以降のテストのためにOracleに1000件データのロードを行う
    *   FixtureBookAddinTest の Oracle__1000件データ を実行し、
        Oracle上の`TypesTable`に1000件のデータをロードする
    *   設定画面で`Oracle.DataAccess.Client`（Oracle 用のプロバイダ）を選択しておく
...
1.  設定画面を開くとプロバイダ名が `Oracle.DataAccess.Client`、
